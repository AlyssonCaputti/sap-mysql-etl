...
A tabela precisa existir antes — quem cria é a migration 002 do dashboard-api.
"""
...
# A ilha do cliente vem da última compra dele da marca delinte — a linha de
# maior emissão, desempatando pelo maior valor_total. Quem nunca comprou
# delinte fica como 'outros'.
#
# Fica verboso porque evito ROW_NUMBER (pra rodar em MySQL 5.x): isolo as
# linhas delinte, acho o par (emissão, valor) máximo de cada cliente e junto
...
_ILHA_DELINTE = f"""
    ...
    FROM (
        ...
        FROM Faturamento
        WHERE UPPER(TRIM(marca)) = 'DELINTE'
    ) d
    JOIN (
        ...
        with conexao() as con:
            ...
            if not cursor.fetchone():
                raise RuntimeError(
                    ...
                    "python dashboard-api/migrations/002_create_faturamento_full.py"
                )
